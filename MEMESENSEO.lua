-- =========================================================================
-- [ MEMESENSE - 完整版 | 彩虹方块 + 弹道追踪 + 手雷透视 + 武器透视 ]
...
local repo = "https://raw.githubusercontent.com/GMC1086/Ben/refs/heads/main/"
...
local Tabs = {
    ...
    SkinChanger = Window:AddTab('皮肤更换', 'paintbrush'),
...
SettingsGroup:AddLabel("菜单按键"):AddKeyPicker("MenuKeybind", {
    Text = "MenuKeybind",
    Default = "RightShift",
    Mode = "Toggle",
...
MiscBox:AddSlider("BhopSpeed", { Text = "连跳速度", Default = 18, Min = 5, Max = 30, Rounding = 1, Suffix = "spd" })
MiscBox:AddToggle("NoFallDamage", { Text = "无坠落伤害", Default = false })
...
MemesenseDepBox:AddToggle("CubeTriggerbot", { Text = "启用触发器自瞄", Default = false })
MemesenseDepBox:AddSlider("CubeTriggerbotDelay", { Text = "触发器延迟", Default = 0.01, Min = 0, Max = 1, Rounding = 3 })
...
HitSoundBox:AddSlider("HitSoundVolume", { Text = "命中音效音量", Default = 1, Min = 0.1, Max = 5, Rounding = 1, Suffix = "x" })
...
CustomCameraBox:AddToggle("CustomFovToggle", {
    Text = "自定义视野",
    Default = false,
}):AddKeyPicker("CustomFovKey", { Text = "自定义视野按键", Default = "One", Mode = "Always" })

CustomCameraBox:AddSlider("FovAmount", { Text = "视野数值", Default = 90, Min = 70, Max = 120, Rounding = 0, Suffix = "deg" })
...
CustomScopeBox:AddToggle("CustomScopeFov", { Text = "自定义瞄准镜视野", Default = false })
CustomScopeBox:AddSlider("ScopeFovValue", { Text = "瞄准镜视野", Default = 70, Min = 10, Max = 100, Rounding = 1, Suffix = "deg" })
...
CustomScopeBox:AddSlider("ScopeCrosshairThickness", { Text = "准星粗细", Default = 2, Min = 1, Max = 10, Rounding = 1, Suffix = "px" })
CustomScopeBox:AddSlider("ScopeCrosshairLengthLR", { Text = "左右长度", Default = 150, Min = 0, Max = 1000, Rounding = 0, Suffix = "px" })
CustomScopeBox:AddSlider("ScopeCrosshairLengthTB", { Text = "上下长度", Default = 100, Min = 0, Max = 1000, Rounding = 0, Suffix = "px" })
...
HitMarkerBox:AddSlider("HitMarkerDuration", { Text = "显示持续时间", Default = 2, Min = 0.5, Max = 5, Rounding = 1, Suffix = "s" })
HitMarkerBox:AddSlider("HitMarkerSpinSpeed", { Text = "旋转速度", Default = 720, Min = 0, Max = 1440, Rounding = 0, Suffix = "deg/s" })
HitMarkerBox:AddSlider("HitMarkerSize", { Text = "Size", Default = 25, Min = 5, Max = 50, Rounding = 0, Suffix = "px" })
HitMarkerBox:AddSlider("HitMarkerThickness", { Text = "粗细", Default = 2, Min = 1, Max = 6, Rounding = 1, Suffix = "px" })
...
task.spawn(function()
    ...
    crosshairGui.Name = "BloxStrike_瞄准镜准星"
...
local function UpdateSkybox(name)
    ...
    local sky = Lighting:FindFirstChild("MEMESENSE_天空")
    if not sky then
        for _, v in pairs(Lighting:GetChildren()) do
            if v:IsA("Sky") then v:Destroy() end
        end
        sky = Instance.new("Sky")
        sky.Name = "MEMESENSE_天空"
        sky.Parent = Lighting
    end
...
local function UpdateWeather(wType)
    ...
    for _, v in pairs(Workspace:GetChildren()) do
        if v.Name == "MEMESENSE_雨滴" then v:Destroy() end
    end
    if wType == "None" then return end

    WeatherPart = Instance.new("Part")
    WeatherPart.Name = "MEMESENSE_天气天空"
    ...
    GroundPart.Name = "MEMESENSE_天气地面"
...
WorldBox:AddSlider("WorldClockTime", { Text = "时钟时间", Default = 12, Min = 0, Max = 24, Rounding = 1, Suffix = "h", Callback = function(_) UpdateLighting() end })
WorldBox:AddToggle("EnableBrightness", { Text = "启用亮度", Default = false, Callback = function(_) UpdateLighting() end })
WorldBox:AddSlider("WorldBrightness", { Text = "亮度", Default = 2, Min = 0, Max = 10, Rounding = 1, Suffix = "x", Callback = function(_) UpdateLighting() end })
...
WorldBox:AddLabel("户外颜色"):AddColorPicker("WorldOutdoorAmbient", {
    Default = Color3.fromRGB(127, 127, 127),
    Title = "户外颜色",
    Callback = function(_) UpdateLighting() end
})
...
for _, gFolder in ipairs(SD.GloveFolders) do
    ...
    if gSkins then
        GlovesBox:AddDropdown("手套皮肤_" .. gName, {
            Text = gName .. " Skin",
            Values = gSkins,
            Default = 1,
...
local CombatBlatantBox = Tabs.Combat:AddRightGroupbox("明目张胆", "zap")
...
BlatantDependencyBox:AddToggle("SilentWallbang", { Text = "穿墙射击", Default = false })
...
BlatantDependencyBox:AddSlider("SilentFovCircleRadius", { Text = "视野圆环半径", Default = 50, Min = 0, Max = 300, Rounding = 0 })
...
BlatantDependencyBox:AddToggle("SilentTeamCheck", { Text = "启用队伍检测", Default = true })
...
RageDependencyBox:AddToggle("RagebotTeamCheck", { Text = "启用队伍检测", Default = true })
...
-- =========================================================================
-- [ 视觉标签页 - ESP 透视系统 ] v5.7 (属性生命值 + 生命条渐变)
-- =========================================================================
...
VisualsESPBox:AddToggle("ESPEnabled",   { Text = "ESP 透视启用", Default = false })
VisualsESPBox:AddToggle("ESPTeamCheck", { Text = "队伍检测", Default = true })

VisualsESPBox:AddDropdown("ESPBoxType", {
    Text    = "方框透视",
    Values  = {"2D 方框","3D 方框","角落方框","Disabled"},
    Default = "2D 方框",
})
...
VisualsESPBox:AddToggle("ESPName", { Text = "名称透视", Default = false })
VisualsESPBox:AddLabel("名称颜色"):AddColorPicker("ESPNameColor", { Default = Color3.new(1,1,1), Title = "名称颜色" })

VisualsESPBox:AddToggle("ESPHealth", { Text = "生命条", Default = false })
VisualsESPBox:AddLabel("生命条顶部颜色"):AddColorPicker("ESPHealthTopColor",       { Default = Color3.fromRGB(0,255,0),   Title = "Health Top"    })
VisualsESPBox:AddLabel("生命条底部颜色"):AddColorPicker("ESPHealthBottomColor", { Default = Color3.fromRGB(255,0,0),   Title = "Health Bottom" })

VisualsESPBox:AddToggle("ESPHealthText", { Text = "生命文本", Default = false })
VisualsESPBox:AddLabel("生命值文本颜色"):AddColorPicker("ESPHealthTextColor",  { Default = Color3.new(1,1,1), Title = "生命值文本颜色" })

VisualsESPBox:AddToggle("ESPDistance", { Text = "距离透视", Default = false })
...
VisualsESPBox:AddToggle("ESPTracer", { Text = "弹道透视", Default = false })
VisualsESPBox:AddLabel("弹道颜色"):AddColorPicker("ESPTracerColor",  { Default = Color3.new(1,1,1)             })
...
VisualsESPBox:AddDropdown("ESPTracerOrigin", {
    Text = "弹道起点", Values = {"Bottom","顶部","中心","鼠标"}, Default = "Bottom",
})

VisualsESPBox:AddToggle("ESPSkeleton", { Text = "骨骼透视", Default = false })
...
local function FindAllTargets()
    ...
    -- 通过 VestDetails 判断是否为队友
    local function isEnemy(char)
        if not char then return false end
        -- 队友与玩家拥有相同的 VestDetails 状态
        local myHasVest = hasVestDetails(lchar)
        local targetHasVest = hasVestDetails(char)
        if myHasVest then
...
BulletImpactV1Part.Name = "方块检测器_物理"
...
local function createTracerBean(startPos, endPos)
    ...
    local style = Options.TracerStyle and Options.TracerStyle.Value or "方块"
    ...
    beamPart.Name = "MEMESENSE_弹道追踪"

    if style == "圆柱体 (Obelius)" then
                    ...
                    for _, hitData in pairs(bullet.Hits) do
                        ...
                        pcall(function()
                            ...
                            if cam and hitData.Position then
                                ...
                                -- 在 endPos 点触发音效和 3D 命中标记
                                if isEnemyHit or targetPart then
                                    PlayHitSound()
                                    
                                    -- 调用你的 3D 命中标记，传递命中点坐标
...
local function isLocalPlayerObject(obj)
    ...
    if obj.Name == "方块检测器_物理" or obj.Name:find("MEMESENSE_") then return true end
    return false
end
...
WorldBoxAtmosphere:AddSlider("AtmosphereDensity", { Text = "大气密度", Default = 0.3, Min = 0, Max = 1, Rounding = 2 })
WorldBoxAtmosphere:AddSlider("Sub-AtmosphereHaze", { Text = "大气雾效", Default = 0, Min = 0, Max = 10, Rounding = 1 })
WorldBoxAtmosphere:AddSlider("AtmosphereGlare", { Text = "大气眩光", Default = 0, Min = 0, Max = 10, Rounding = 1 })
WorldBoxAtmosphere:AddToggle("EnableColorCorrection", { Text = "启用色彩校正", Default = false })
WorldBoxAtmosphere:AddSlider("SaturationSlider", { Text = "饱和度", Default = 0, Min = -1, Max = 1, Rounding = 2 })
WorldBoxAtmosphere:AddSlider("ContrastSlider", { Text = "对比度", Default = 0, Min = -1, Max = 1, Rounding = 1 })
...
cfgDefault("手雷透视",     false)
cfgDefault("GrenadeTracers", false)
cfgDefault("彩色烟雾",   false)
cfgDefault("NoSmoke",        false)
cfgDefault("烟雾颜色",     Color3.fromRGB(255, 0, 0))
...
task.spawn(function()
    ...
    -- 通过 VestDetails 进行队伍检测
    -- 如果你拥有 VestDetails → 你是 CT
    -- 仅对没有 VestDetails 的人应用透视材质 (T)
    -- и наоборот
...
-- 将所有内容放入 task.spawn，避免在脚本根目录创建局部变量
task.spawn(function()
